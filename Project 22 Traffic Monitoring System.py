import tkinter as tk
from tkinter import ttk
import cv2
import numpy as np
from PIL import Image, ImageTk

class TrafficMonitoringApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Project 22: Traffic Monitoring System")
        self.root.geometry("900x650")

        self.cap = None
        self.is_running = False
        self.fgbg = cv2.createBackgroundSubtractorMOG2(history=500, varThreshold=50)

        toolbar = ttk.Frame(self.root, padding=8)
        toolbar.pack(side=tk.TOP, fill=tk.X)

        self.btn = ttk.Button(toolbar, text="Start Feed", command=self.toggle_stream)
        self.btn.pack(side=tk.LEFT, padx=4)

        self.status = ttk.Label(toolbar, text="Congestion: Idle", font=("Arial", 11, "bold"))
        self.status.pack(side=tk.LEFT, padx=20)

        self.display = ttk.Label(self.root)
        self.display.pack(expand=True, fill=tk.BOTH, padx=10, pady=10)

    def toggle_stream(self):
        if not self.is_running:
            self.cap = cv2.VideoCapture(0)
            if self.cap.isOpened():
                self.is_running = True
                self.btn.configure(text="Stop Feed")
                self.loop()
        else:
            self.is_running = False
            if self.cap:
                self.cap.release()
            self.btn.configure(text="Start Feed")

    def loop(self):
        if not self.is_running:
            return

        ret, frame = self.cap.read()
        if ret:
            h, w, _ = frame.shape
            fgmask = self.fgbg.apply(frame)

            # Analyze traffic density by measuring occupied white pixels in motion mask
            density = np.sum(fgmask > 0) / (h * w)

            if density < 0.05:
                label = "Light Flow"
                color = (0, 255, 0)
                ui_color = "green"
            elif density < 0.18:
                label = "Moderate Flow"
                color = (0, 255, 255)
                ui_color = "orange"
            else:
                label = "Heavy Traffic / Jam"
                color = (0, 0, 255)
                ui_color = "red"

            self.status.configure(text=f"Congestion: {label} ({density*100:.1f}%)", foreground=ui_color)
            cv2.putText(frame, f"Condition: {label}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)

            rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            pil_img = Image.fromarray(rgb)
            pil_img.thumbnail((850, 550))
            self.tk_photo = ImageTk.PhotoImage(pil_img)
            self.display.configure(image=self.tk_photo)

        self.root.after(20, self.loop)

if __name__ == "__main__":
    root = tk.Tk()
    app = TrafficMonitoringApp(root)
    root.protocol("WM_DELETE_WINDOW", lambda: (setattr(app, 'is_running', False), root.destroy()))
    root.mainloop()